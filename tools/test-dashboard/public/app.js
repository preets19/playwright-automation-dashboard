...
function buildRecorderAnalysisPrompt(input) {
  return [
    ...
    '',
    'Free-text field length: every `reason`, `evidence`, `reviewNote`, `note`, `intentHint`, and `uiContextHint` field must be a fragment of 15 words or fewer — state the decision and its key cause, not a full sentence or narration. EXCEPTION: `assumptions[]` and `ambiguities[]` entries (the `statement`/`question`/`note` text within them) are NOT capped — these carry surprising or multi-rule findings and need room; keep them as complete as needed.',
    '',
    'Omit empty/default fields rather than emitting them. Specifically: do not emit a field whose value is an empty string, an empty array, or null UNLESS a downstream consumer requires its presence. Consumers treat an absent field as empty/none. This applies to fields like existingArtifact, helperRef (on non-helper locators), params, imports, fieldName, finalLocatorExpression when empty. Do NOT omit: any enum field (operationType, classification, status, assertionRole, triggerClass — these stay explicit even when "none"), candidateLocators when non-empty, sourceOperationOrders, or any structured selector data.',
    '',
    ...
    '- Classify triggerClass now (mechanical, from function + role + accessible name — this is a fact feeding Phase C\'s transition hierarchy, not a boundary decision):',
    '  - navCapable: role=link click; nav/menu item leading elsewhere; button whose name advances a flow (Next, Continue, Proceed, Checkout, Login, Sign In, Submit-that-advances, Go-to/View a destination); explicit goto/redirect.',
    '  - inPageControl: Apply, Filter, Sort, Search, Refine, Update; pagination; expand/collapse; show-more; tab-switch within a page; a row/card click opening a panel/drawer in place.',
    '  - overlayOpener: a click opening a tooltip/popover/dropdown/menu/listbox (target or resulting container matches an overlay tell: id /tippy|popover|menu|tooltip/, or aria role menu/dialog/listbox).',
    '  - none: fill, select, check, type, focus-only, incidental/duplicate clicks, assertions, waits.',
    '- Decompose the locator if it has chained parts (see locator rules below). Do this now, per-operation.',
    '- Mark isMeaningful. An incidental click remains operationType click; mark isMeaningful false only when it does not trigger meaningful UI state (focus-only clicks, duplicate clicks, Tab presses, arrow-key corrections). When isMeaningful is false, also add a recorderNoise entry.',
    '- Do NOT classify assertionRole yet — leave it for Phase D. Do NOT decide dataCandidates yet — leave it for Phase E. Do NOT decide uiBoundaries yet — leave it for Phase C.',
    '',
    ...
    '',
    '---',
    'PHASE C — Identify UI boundaries (deterministic transition hierarchy)',
    'Walk the operations once, in order, maintaining currentPage state (the entry from Phase A is the first currentPage). At each operation decide whether a page transition occurs using the hierarchy below. Check tiers in order; STOP at the first tier whose guard passes. A tier whose guard fails does not match — fall through.',
    '',
    '  TIER 1 — URL change. A goto, waitForURL, or recorded URL delta is present.',
    '    GUARD: the path changed. A query-param-only or hash-only change is NOT a transition (same-page filter/state) — fall through.',
    '    Match => transition, boundaryMechanismConfidence "certain".',
    '  TIER 2 — Trigger + confirmation. A navCapable trigger (Phase B triggerClass) occurred since the last transition, AND a page-level assertion follows it (childTarget is h1, heading role, nav, or main/landmark) whose asserted content is NEW (not equal to currentPage\'s established identifier).',
    '    GUARD: the trigger was navCapable — NOT inPageControl and NOT overlayOpener. (This is what stops Apply/Filter clicks and tippy/menu clicks from creating false transitions.)',
    '    Tier 2 determinism rule — confirmation must be immediate, not retroactive: the confirming page-level assertion must be the FIRST assertion encountered after the navCapable trigger, before any other operation intervenes. If the first assertion after the trigger is NOT page-level (generic container, ordinal selector, or any non-page-level target), Tier 2 does NOT match — even if a stronger, page-level assertion (e.g. a heading) appears later in the trace, after other operations have occurred. A later assertion never retroactively upgrades an earlier weak confirmation to Tier 2. If you find yourself reasoning \'the immediate confirmation is weak, but a later assertion proves the transition\' — STOP. That reasoning pattern is exactly what this rule forbids. Fall through to Tier 3 (or DEFAULT) using the immediate evidence only. The later strong assertion may still be used for pageIdentifier resolution (R1/R2), but it must NOT change the transitionTier, which is fixed by the immediate confirmation alone.',
    '    Match => transition, boundaryMechanismConfidence "confirmed".',
    '  TIER 3 — Corroboration (only when Tiers 1 and 2 do not match). A distinctly-new page-level landmark cluster appears AND no inPageControl trigger since the last transition explains it.',
    '    Match => transition, boundaryMechanismConfidence "ambiguous"; add a matching ambiguities entry naming the competing same-page interpretation.',
    '  DEFAULT — no tier matches => SAME PAGE. The operation belongs to currentPage. This covers content-change (post-Apply re-assertion of the same identifier), overlay interactions, and mid-page assertions.',
    '',
    'Bias rule (asymmetric risk): when a navCapable trigger is present but confirmation is weak/ambiguous, prefer a transition (confidence "ambiguous") over merging — an over-split page is a minor cleanliness cost; a missed transition merges two real pages and corrupts ownership. Log the assumption via an ambiguities entry.',
    '',
    'Seam ownership: the triggering action belongs to the SOURCE page (you click "Next" while still on the source page); the confirming assertion opens and belongs to the DESTINATION page (it is that page\'s readiness evidence).',
    '',
    'For every transition emit a uiBoundary. A DOM overlay/popover is NOT a browserPopup and is NOT a page boundary — overlay interactions are same-page (DEFAULT branch), never boundaries. Do not infer page objects, classes, or ownership — boundaries are observations. Reference evidenceOperationOrders from the trace.',
    '',
    'Page identifier (deterministic readiness signal — emit on each uiBoundary as pageIdentifier fields):',
    '- pageIdentifierSelector + pageIdentifierText = the page-level confirming assertion\'s locator + expected text for the destination page (for the entry page\'s own boundary entry, the first page-level assertion observed).',
    '- pageIdentifierQuality: strong (stable page-level selector with intrinsic page-specific text, e.g. an h1 title unique to the page) | weak (generic/ordinal/class-only, or a selector shared across multiple pages) | missing (no page-level assertion confirms the page before its next interaction).',
    '- Resolve weak/missing automatically — never ask; decide and record an ambiguities entry:',
    '  R1 prefer the STRONGEST page-level identifier anywhere in the trace for that page, even if asserted late; a stronger later heading demotes an earlier weak one.',
    '  R2 if the immediate confirmation is overlay/generic but a page-level heading for the same page appears later, use the later heading and note it was asserted late (may need on-arrival check; impact medium).',
    '  R3 if NO page-level identifier exists, fall back: (a) first page-level landmark after the transition (h1>heading>main>nav); else (b) first asserted element after the trigger; else (c) the triggering click\'s destination. Record an ambiguity with impact.',
    '  R4 entry page with only an app-shell identifier (site-wide nav present on every page): accept as-is — reached by goto, disambiguation unnecessary. No ambiguity needed.',
    '',
    '---',
    ...
    '',
    'Readiness-gap rule:',
    '- Many state-changing operations (clicks that navigate, open a view, or trigger a transition) will have no recorded assertion confirming the destination loaded before the next interaction proceeds — the recording simply continues. This is common and is not itself an error.',
    ...
    '- For every Phase C transition trigger that has no assertion directly confirming its destination state before the next meaningful operation, add an entry to readinessGaps: { "triggerOperationOrder": 1, "expectedReadyState": "", "nextOperationOrder": 2, "reason": "" }',
    '- This applies uniformly regardless of transition type (route, tab, panel, modal, etc.) — the rule is about absence of evidence, not transition type.',
    ...
    '- Capture stateTransitions for the Phase C transitions. Describe the strongest recorded destination evidence. Record an ambiguity when a generic signal could also be satisfied in the source state — do not invent stronger evidence than what\'s recorded.',
    '- Confirm recorderNoise entries from Phase B are complete (final sweep, not a new classification pass).',
    '',
    'Readiness-scope rule (compute this last in Phase F, after Phase D\'s assertionRole values are final, before Phase G):',
    '- For every operation in operationTrace, compute readinessScope: the NEAREST preceding operation whose assertionRole is "readiness" (or null if none precedes it), carrying that assertion\'s sourceOperationOrder and its locator (parentScope + childTarget, exactly as decomposed in Phase B). Only the nearest one — an earlier readiness scope is superseded the moment a nearer one exists, and is not a valid fallback to compute here.',
    '- If no readiness assertion precedes an operation, readinessScope is null — a valid, expected case meaning the scope is the whole page, not an error.',
    '- This value is independent of whether the operation\'s OWN locator has a recorded parentScope. Both apply simultaneously — an operation can have its own dynamic parent AND a readinessScope; both are facts to report, not alternatives.',
    '',
    '---',
    'PHASE G — Final integrity pass (self-check before output)',
    'Before producing output, verify:',
    '- Every operation in operationTrace has exactly one operationType and one triggerClass, assigned once (Phase B), and exactly one assertionRole if applicable (Phase D only).',
    '- Every uiBoundary corresponds to a Tier 1/2/3 match, references valid evidenceOperationOrders, and has a boundaryMechanismConfidence; every "ambiguous" boundary has a matching ambiguities entry. No overlay interaction is recorded as a boundary. Every uiBoundary carries pageIdentifierSelector/pageIdentifierText/pageIdentifierQuality (with an ambiguity when resolved from weak/missing).',
    '- Tier 2 self-audit: for every uiBoundary with transitionTier \'2\', verify that its evidenceOperationOrders\' confirming assertion is the operation IMMEDIATELY following the trigger operation, with no other operations between them. If any other operation (assertion or action) occurs between the trigger and the confirming assertion, this boundary VIOLATES the Tier 2 rule — change transitionTier to \'3\' and boundaryMechanismConfidence to \'ambiguous\' before producing output. Do this check for every Tier 2 boundary, not just ambiguous-looking ones.',
    '- successCoverage is populated per Phase D precedence — not left default/empty.',
    '- readinessGaps lists every Phase C transition trigger lacking a directly confirming assertion before the next meaningful operation.',
    ...
    '  "operationTrace": [{ "order": 1, "rawOperation": "", "operationType": "goto | click | fill | select | check | assertion | popup | wait", "triggerClass": "navCapable | inPageControl | overlayOpener | none", "assertionRole": "readiness | success | intermediate | none", "uiContextHint": "", "rawLocator": "", "locatorParts": { "parentScope": "", "parentScopeRisk": "stable | dynamic | broad | none | unknown", "childTarget": "", "childTargetType": "role | text | label | testId | css | xpath | unknown", "childTargetValue": "" }, "value": "", "intentHint": "", "isMeaningful": true, "riskFlags": [], "candidateFallback": { "strategy": "useAsIs | stripDynamicParent | scopeRequired | firstMatchProvisional | block | none", "candidateLocator": "", "reason": "" }, "readinessScope": { "sourceOperationOrder": null, "parentScope": "", "childTarget": "" } | null }],',
    ...
    '  "uiBoundaries": [{ "observedLabel": "", "boundaryType": "routeChange | fullScreenState | browserPopup | modal | drawer | popover | menu | tab | panel | wizardStep | headingState | unknown", "boundaryMechanismConfidence": "certain | confirmed | ambiguous", "transitionTier": "1 | 2 | 3", "pageIdentifierSelector": "", "pageIdentifierText": "", "pageIdentifierQuality": "strong | weak | missing", "evidenceOperationOrders": [], "reason": "" }],',
...
function buildFrameworkMappingPrompt(input, contextBundle) {
  return [
    ...
    '',
    'Entry contract (applies to any app, not a special case — application entry is always setup, never part of any workflow):',
    '- The entry navigation (initial goto, or auth-session entry link) PLUS the first readiness assertion confirming the landing page are ALWAYS setupOperationOrders, never behaviorOperationOrders — regardless of which page is landed on (unauth home, auth home, or any auth-session landing).',
    '- Setup ends at the first readiness assertion confirming the landing page. Every operation after it is behavior.',
    '- Uniform across entry types: whether the journey continues unauthenticated, performs login as its first action, or begins already authenticated, the rule is identical — entry navigation + landing assertion = setup; first action after = first behavior.',
    '- Entry navigation is the test\'s setup step (e.g. open(baseUrl)), not a workflow action. The landing assertion confirms setup succeeded; it is not a workflow\'s readiness.',
    '- If no landing assertion was recorded, entry navigation alone is setup; add an assumption flagging the landing was not confirmed.',
    '',
    'Base URL: the first recorded goto is the entry point (entry.recordedUrl). Generated code must read the base URL via contextBundle.configAccess.baseUrl.expression — never as a literal string. If contextBundle.configAccess.available is false, flag this as a blocking gap (do not invent or guess a symbol). runtimeInputs.value still carries the actual URL string for traceability only — never emit it literally in generated code. sourceRef should be set to contextBundle.configAccess.baseUrl.expression when available.',
    '',
    '---',
    ...
    '  3. If Prompt 1\'s readinessScope is not null, first apply a plausibility guard: skip this tier entirely when the operation\'s own childTarget is a page-level element (h1, any heading role/tag, nav, main, or a top-level container such as #main-container or an equivalently page-scoped landmark) — a page-level element cannot be a descendant of a widget, overlay, filter section, or heading, so scoping it under readinessScope\'s locator would be DOM-implausible. Only build the candidate when the childTarget plausibly sits inside the readiness ancestor (a control, input, link, or item within that section). When the guard skips this tier, fall straight through to Tier 4 — do not substitute a different candidate in its place. When the guard does not skip it: build one candidate, the operation\'s own childTarget scoped under readinessScope\'s locator (parentScope + childTarget combined as an ancestor-descendant locator). Before adding it to the candidate list, check whether it is exactly string-identical to any candidate already added from Tier 1 or Tier 2 — if so, skip it (already covered, no need to duplicate). This tier contributes at most one candidate, never more.',
    ...
    '- When a Prompt 1 uiBoundary carries a pageIdentifierSelector, use it as the readiness signal for that boundary\'s destination page owner — do not re-derive it from other recorder evidence. Match the boundary to its owner by the operation orders it governs.',
    '- A boundary\'s pageIdentifier populates readinessEvidence ONLY. It is never added to any operation\'s locatorPlan candidate list, and a Phase D locator scope (parentScope/childTarget) is never promoted into a page readiness identifier.',
    '- A uiBoundary whose pageIdentifierQuality is "missing" falls through to the strongest-destination-evidence derivation above. Phase D\'s readinessScope mechanism is unchanged — it still selects the nearest preceding readiness operation as a scoping ancestor; only the identifier value it carries is now cleaner.',
...
function buildArtifactDesignPrompt(input, contextBundle) {
  return [
    ...
    '   - locators[].status carries Prompt 2\'s classification forward verbatim: stable | resolvableViaHelper | provisional | blocked.',
