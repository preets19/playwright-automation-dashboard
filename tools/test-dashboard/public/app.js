...
function buildRecorderAnalysisPrompt(input) {
  return [
    ...
    '  "interactionTypes": [{ "type": "", "evidence": [], "setupOrBehavior": "setup | behavior" }],',
    '  "recordedAssertions": [],',
    '  "inferredSuccessCriteria": [],',
    '  "readinessSignals": [{ "pageOrStep": "", "signal": "", "reason": "" }],',
    '  "stateTransitions": [{ "triggerAction": "", "expectedReadyState": "", "waitOwnerHint": "page | workflow | test" }],',
    '  "recorderNoise": [],',
    '  "ambiguities": []',
    '}',
    '```',
    ...
    '- Classify recorded interactions as setup or behavior under test.',
    '- Identify reusable interaction types such as click, fill, select, checkbox, radio, upload, menu item, modal, tab, combobox, grid, toast, or wizard step.',
    '- Identify readiness signals such as headings, editable fields, hidden spinners, enabled buttons, URL/path changes, loaded result counts, updated cart counts, modal visibility, or confirmation text.',
    '- Identify state transitions where a workflow/page should wait before the next action.',
...
function buildFrameworkMappingPrompt(input, contextBundle) {
  return [
    ...
    '  "naming": { "files": [], "classes": [], "methods": [], "locatorFields": [], "risks": [] },',
    '  "setupVsBehavior": [],',
    '  "assertionStrategy": [],',
    '  "readiness": { "pageMethods": [], "workflowWaits": [], "fallbackRisks": [] },',
    ...
    '- Apply the naming contract while proposing artifacts: camelCase files, methods, variables, and locator fields; PascalCase classes and types; full element-type suffixes; no btn/dd/ddl/txt/lbl/msg prefixes.',
    '- Decide whether each recorded action is setup or behavior under test. Setup can use a stable direct route or existing workflow when app conventions allow it.',
    '- Decide assertion strategy early, including whether each assertion input should be a string, boolean, number, typed object, or page-owned assertion helper.',
...
function buildArtifactDesignPrompt(input, contextBundle) {
  return [
    ...
    '- Define wait ownership: page readiness, workflow transition, final state collection, or test assertion.',
    '- Preserve the naming contract: camelCase files/methods/locators, PascalCase classes/types, full element suffixes, no abbreviated UI prefixes.',
    '',
    guidedRequestSection(input),
    '',
    'Recorder Analysis:',
