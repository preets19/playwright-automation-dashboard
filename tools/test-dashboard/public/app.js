...
function buildRecorderAnalysisPrompt(input) {
  return [
    ...
    '- Classify UI ownership from recorded state boundaries: a page is a distinct route or full-screen application state with its own identity/readiness; a component is a region within a page that does not establish an independent screen state. Preserve headers, navigation regions, menus, dropdowns, popovers, sidebars, tabs, modals, drawers, and panels as their component/overlay type unless the recording proves a distinct page state. Opening a component is a component state transition; an action inside it may separately transition to a destination page. A DOM overlay/popover is not a browserPopup.',
...
function buildFrameworkMappingPrompt(input, contextBundle) {
  return [
    ...
    '- Page: owns a distinct route or full-screen application state with independent identity and readiness.',
    '- Component: owns a reusable or transient region within a page, including navigation, menus, dropdowns, panels, tabs, modals, and drawers; visibility changes alone do not make it a page.',
    ...
    '- Define each meaningful destination state from the strongest recorded evidence and preserve Prompt 1 ownership types: only a distinct route or full-screen state with independent identity/readiness maps to a page; nested or transient UI regions remain components. A readiness signal must not already be satisfied in the source state; use recorded destination text, route state, or a destination-owned control when available. Keep materially different page states distinct even when the recorder used the same generic locator, and mark unsupported identity as provisional or blocked according to risk.',
...
function buildArtifactDesignPrompt(input, contextBundle) {
  return [
    ...
    '- Preserve the mapper ownership type in artifact design. Only page owners use the Page suffix and represent distinct route/full-screen states; component owners use a semantic suffix such as Navigation, Header, Menu, Panel, Tab, Modal, or Drawer and must not be promoted to pages merely because their visibility changes. Pages/components own locators, navigation, UI actions, and state-specific readiness while preserving inherited framework method signatures. Keep framework hooks such as parameterless waitUntilReady() compatible; when readiness requires runtime expected data, use a distinct state-specific method or return observed state for the spec instead of changing the inherited hook.',
...
function buildCodeGenerationPrompt(input, contextBundle) {
  return [
    ...
    '',
    'Execution rules:',
    '- Before writing, perform a structural preflight only: verify declared files and symbols agree, page/component class names preserve their contracted ownership type, locator fields/factories and parameters are internally consistent, required readiness and assertion inputs exist, imports are concrete, source text is valid, and workflowOperationOwnership covers meaningful operations exactly once without overlap. Stop only for a genuine contradiction that cannot be implemented exactly; do not redesign or reclassify an approved owner, locator, state, assertion, or workflow assignment.',
